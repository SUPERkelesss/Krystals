...
with open('spacegroup_hall_mapping.json') as f:
    mapping = json.load(f)
...
content = '\n'.join(parts)
with open('crystal-core/src/main/kotlin/com/krystals/core/SpaceGroupCatalog.kt', 'w', encoding='utf-8') as f:
    f.write(content)

print(f'Wrote SpaceGroupCatalog.kt ({len(content)} chars, {len(symbols)} space groups)')
