...
const val MAX_LIGHT_ELEVATION_DEGREES = 89.9f
 ...
 * Both shares are injected into every material (lit atom PBR via the scene's
 * IndirectLight + directional light; hand-lit bond/mesh shaders via the sunShare
 * material parameter), so atom, bond and mesh instances always follow one light model.
 *
 * Sun parameters:
 *  - [azimuthDegrees]  sun azimuth (light angle)
 *  - [elevationDegrees] sun elevation (light height)
 *  - [intensity]        sun strength
 *  - [diffusion]        sun angular radius / highlight spread
 *
...
data class WorldLight(
    val azimuthDegrees: Float = 150f,
    val elevationDegrees: Float = 30f,
    val intensity: Float = 0.5f,
    val diffusion: Float = 0.9f,
) {
    init {
        require(elevationDegrees in 0f..MAX_LIGHT_ELEVATION_DEGREES) { "light elevation must be between 0 and 89.9 degrees" }
...
data class ViewerAppearance(
    ...
    // Per v0.7.0: hydrogen-bond appearance (radius in Å, opacity 0..1).
...
object RenderPalette {
    ...
    fun vestaArgb(symbol: String): Long = when (symbol) {
        // Keep hydrogen as a subdued neutral, leaving headroom for its shared atom
        // highlight so it does not read as clipped white beside colored atoms.
        "H" -> 0xFFC4C4C4; "C" -> 0xFF505050; "N" -> 0xFF3050F8; "O" -> 0xFFFF0D0D
